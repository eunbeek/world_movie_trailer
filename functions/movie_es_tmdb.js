 ...
 */
async function fetchMovieListFromTMDBByES() {
  const movies = [];
  ...
  async function fetchMovies(page) {
    try {
      const responseRun = await fetchRunningMovieByCountryCode("ES", "es-ES", page);
      const responseUp = await fetchUpcomingMovieByCountryCode("ES", "es-ES", page);
