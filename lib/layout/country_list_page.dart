  ...
  Future<void> _fetchSpecialMovies() async {
    try {
      final settingsProvider =
          Provider.of<SettingsProvider>(context, listen: false);
      List<Movie> movies =
          await MovieService.fetchMovie(special, settingsProvider.language);

      if (movies.isEmpty) {
        if (mounted) {
          setState(() {
            specialSection = null;
            specialMovieList = const [];
          });
        }
        return;
      }
      ...
      if (settingsProvider.lastSpecialNumber != 0) {
        ...
        final daysDifference =
            now.difference(settingsProvider.lastSpecialFetched!).inDays;
        ...
        if (daysDifference > 7) {
          ...
          var filteredMovies = movies
              .where((movie) =>
                  movie.period == settingsProvider.lastSpecialNumber + 1)
              .toList();
          ...
          if (filteredMovies.isNotEmpty) {
            ...
            settingsProvider.updateLastSpecialNumber(
                settingsProvider.lastSpecialNumber + 1);
          } else {
            // 새로운 영화가 없으면 최신 영화로 설정
            var latestMovies = movies
                .where((movie) => movie.period == movies[0].period)
                .toList();
            setState(() {
          ...
          var filteredMovies = movies
              .where(
                  (movie) => movie.period == settingsProvider.lastSpecialNumber)
              .toList();
        ...
        var latestMovies =
            movies.where((movie) => movie.period == movies[0].period).toList();
        setState(() {
          specialSection = latestMovies[0];
          specialMovieList = latestMovies;
        });
      ...
      body: SafeArea(
        child: Stack(
          children: [
            ...
            Column(
              children: [
                if (hotFixMode) HotFixBanner(language: languageCode),
                Container(
                              ...
                              children: [
                                if (isEditMode) ...[
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () {
                                      setState(() {
                                        if (oldCountryOrder != null)
                                          settingsProvider.updateCountryOrder(
                                              oldCountryOrder!);
                                        oldCountryOrder = null;
                                        isEditMode = false;
                                      });
                                  ...
                                  IconButton(
                                    icon: Image.asset(
                                      settingsProvider.isDarkTheme
                                          ? 'assets/images/dark/icon_store_DT_xxhdpi.png'
                                          : 'assets/images/light/icon_store_LT_xxhdpi.png',
                                      height: iconSize,
                                      width: iconSize,
                                    ),
                                    onPressed: () {
                            ...
                            IconButton(
                              onPressed: () async {
                                ...
                                if (await canLaunchUrl(
                                  ...
                                  await launchUrl(Uri.parse(promotionUrl!),
                      ...
                      : ReorderableListView(
                          ...
                          onReorder: (int oldIndex, int newIndex) {
                            setState(() {
                              ...
                              if (newIndex <= boxIndex)
                                newIndex = boxIndex + 1; // 'box' 앞으로 이동 금지
                                ...
                                child: Material(
                                  ...
                                  child: InkWell(
                                    onTap: () {
                                      ...
                                      if (index == 0)
                                        LogHelper()
                                            .logEvent('box_office_clicked');
                                      LogHelper().logEvent('country_clicked',
                                          parameters: {
                                            ...
                                            children: [
                                              ...
                                              if (!isEditMode &&
                                                  localizedCountries[
                                                          languageCode] !=
                                                      null &&
                                                  settingsProvider.getCountryStatus(
                                                      localizedCountries[
                                                              languageCode]!
                                                          .entries
                                                          .firstWhere(
                                              ...
                                              if (isEditMode &&
                                                ...
                                                Positioned(
                                                  ...
                                                  child: Image.asset(
                                                    settingsProvider.isDarkTheme
                                                        ? 'assets/images/dark/icon_drag_handle_DT_xxhdpi.png'
                                                        : 'assets/images/light/icon_drag_handle_LT_xxhdpi.png',
                                                    height: iconSize,
                                                    width: iconSize,
                                                  ),
                                                ),
                    ...
                    child: InkWell(
                      onTap: () {
                        ...
                        if (settingsProvider.isQuotes) {
                          _unmarkNewOnExit();
                          LogHelper().logEvent(
                            'special_quotes_clicked',
                            parameters: {'section_name': 'quote'},
                          );
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                          ...
                          _unmarkNewOnExit();
                          LogHelper().logEvent(
                            'special_movie_clicked',
                            parameters: {
                              'section_name': specialSection!.special
                            },
                          );
                          Navigator.push(
                            context,
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            settingsProvider.isQuotes
                                ? getSpecialQuoteLable(languageCode)
                                : getSpecialLable(
                                    specialSection!, languageCode),
                            style: TextStyle(
...
class GradientBorderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final gradient = LinearGradient(
      colors: isDark
          ? [const Color(0xff12d6df), const Color(0xfff70fff)]
          : [const Color(0xff00ffed), const Color(0xff9d00c6)],
